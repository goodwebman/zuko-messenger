...
function MessageStatus({ read }: { read: boolean }) {
  return (
    ...
    >
      ...
      {read && <Check className="-ml-1.75 size-3" />}
    </span>
  );
}
...
export function MessageBubble({ message, mine }: MessageBubbleProps) {
  return (
    <div className={cn('flex animate-message-in', mine ? 'justify-end' : 'justify-start')}>
      <div
        className={cn(
          'max-w-[75%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed shadow-e1',
          mine
      ...
      >
        ...
        <span className="mt-1 flex items-center justify-end gap-1 text-[10px] text-fog-text">
